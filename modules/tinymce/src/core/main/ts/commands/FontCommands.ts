...
export const fontSizeQuery = (editor: Editor) => mapRange(editor, (elm: SugarElement<Element>) => {
  if (elm.dom.nodeName === 'A' && elm.dom.getAttribute('mode') === 'insert') {
    return FontInfo.getFontSize(editor.getBody(), elm.dom.querySelector('.editable-font'));
  }
  return FontInfo.getFontSize(editor.getBody(), elm.dom);
}).getOr('');
