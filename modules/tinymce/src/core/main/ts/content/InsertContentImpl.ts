...
const validInsertion = function (editor: Editor, value: string, parentNode: Element) : Element {
...
export const insertHtmlAtCaret = function (editor: Editor, value: string, details) {
  ...
  // 给图片外面套一个父元素，用来清除左右对齐造成的浮动
  const temp = document.createElement('div');
  temp.innerHTML = value;
  if (temp.firstElementChild?.tagName === 'IMG') {
    temp.classList.add('tox-clear-float');
    value = temp.outerHTML;
  }
  ...
  if (!parserArgs.invalid) {
    ...
    const parseHtml = validInsertion(editor, value, parentNode);
    LinkTool.afterInsertLink(editor, parseHtml);
  } else {
