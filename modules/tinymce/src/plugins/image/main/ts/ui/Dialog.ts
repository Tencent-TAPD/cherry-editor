...
import {
  normalizeCss as doNormalizeCss,
  getSelectedImage
} from '../core/ImageSelection';
...
interface Helpers {
  ...
  imageComputedSize: (editor: Editor) => Size;
...
const makeDialog = (helpers: Helpers, editor: Editor) => (info: ImageDialogInfo): DialogType.DialogSpec<ImageDialogData> => {
  const state = createState(info);
  if (!/^\d+$/.test(info.image.width)) {
    const dimensions = helpers.imageComputedSize(editor);
...
const imageComputedSize = function (editor) {
  return function () {
    const imgElm = getSelectedImage(editor);
    const width = String(Math.round(+window.getComputedStyle(imgElm).getPropertyValue('width')
    .replace('px', '')) || '');
    const height = String(Math.round(+window.getComputedStyle(imgElm).getPropertyValue('height')
      .replace('px', '')) || '');
    return {
      width,
      height
...
export const Dialog = (editor: Editor) => {
  ...
  const open = () => {
    collect(editor)
      .then(makeDialog(helpers, editor))
