...
import CherryDialog from './CherryDialog';

const handleSubmit = (editor: Editor, info: LinkDialogInfo, elm: Element) => (api: Dialog.DialogInstanceApi<LinkDialogData>) => {
  ...
  DialogConfirms.preprocess(editor, changedData).then((pData) => {
    Utils.link(editor, attachState, pData, elm);
  });
...
const getInitialData = (info: LinkDialogInfo, defaultTarget: Optional<string>, editor: Editor): LinkDialogData => {
...
const makeDialog = (settings: LinkDialogInfo, onSubmit, editor: Editor): Dialog.DialogSpec<LinkDialogData> => {
  ...
  const initialData = getInitialData(settings, defaultTarget, editor);
...
const open = function (editor: Editor) {
  ...
  data.then((info) => {
    ...
    const onSubmit = handleSubmit(editor, info, elm);
    return makeDialog(info, onSubmit, editor);
  }).then((spec) => {
    // editor.windowManager.open(spec);
    CherryDialog.show(spec, editor);
  });
};
