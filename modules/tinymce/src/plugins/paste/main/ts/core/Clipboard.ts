...
const hasWXWorkImage = function (content) {
  return hasContentType(content, 'text/html') && /file:\/\/\/.*WXWork/.test(content['text/html']);
};

const isPasteSingleImage = function (clipboardContent) {
  const singlePattern = /^<img src="file:\S+" \/>$/;
  let content = '';
...
const pasteImage = (editor: Editor, imageItem: FileResult) => {
  ...
  if (isValidDataUriImage(editor, img)) {
    ...
    img.onload = function() {
      ...
      const width = img.width > clientWidth * 0.8 ? ' width="80%"' : '';
      pasteHtml(editor, `<img src="${blobInfo.blobUri()}"${width}">`, false);
    };
  } else {
...
const getImagesFromDataTransfer = (editor: Editor, dataTransfer: DataTransfer, clipboardContent: ClipboardContents): File[] => {
  const items = dataTransfer.items ? Arr.filter(Arr.map(Arr.from(dataTransfer.items), (item) => { 
    // fix chrome91 paste image from WeCom
...
const getVideosFromDataTransfer = function (dataTransfer: DataTransfer) {
  const items = dataTransfer.items ? Arr.map(Arr.from(dataTransfer.items), item => item.getAsFile()) : [];
  const files = dataTransfer.files ? Arr.from(dataTransfer.files) : [];
  const videos = Arr.filter(items.length > 0 ? items : files, file => file && /^video\/(mp4|quicktime|x-ms-wmv|x-msvideo|x-matroska|x-flv)$/.test(file.type));
  return videos;
};

const pasteImageOrVideoData = function (editor: Editor, e: ClipboardEvent | DragEvent, rng: Range) {
  ...
  if (Settings.getPasteDataImages(editor) && dataTransfer) {
    ...
    const images = getImagesFromDataTransfer(editor, dataTransfer, clipboardContent);
    if (images.length > 0) {
      e.preventDefault();
  ...
  const filePickerCallback = Settings.getFilePickerCallback(editor);
  if (filePickerCallback && dataTransfer) {
    const files = getVideosFromDataTransfer(dataTransfer);
    if (files.length > 0) {
      e.preventDefault();
...
const registerEventHandlers = (editor: Editor, pasteBin: PasteBin, pasteFormat: Cell<string>) => {
  ...
  function insertClipboardContent(editor: Editor, clipboardContent: ClipboardContents, isKeyBoardPaste: boolean, plainTextMode: boolean, internal: boolean) {
    ...
    if (plainTextMode) {
      const urlReg = /(((?:[a-z][a-z0-9+.-]{1,31}:)?)((?:(?:\/\/)(?:\S+(?::\S*)?@)?(?:(?:1\d\d|2[01]\d|22[0-3]|[1-9]\d?)(?:\.(?:1?\d{1,2}|2[0-4]\d|25[0-5])){2}(?:\.(?:1\d\d|2[0-4]\d|25[0-4]|[1-9]\d?))|(?![-_])(?:[-\w\xa1-\xff]{0,63}[^-_]\.)+(?:[a-zA-Z\xa1-\xff]{2,}\.?))(?::\d{2,5})?(?:[/?#][^\s<>\x00-\x1f\u4e00-\u9fa5"()]*)?)|(?:[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*)))/ig;
        if (content.match(urlReg) === null) {
          pasteText(editor, content);
        } else {
          const encodedText = editor.dom.encode(content).replace(/\r\n/g, '\n');
          const normalizedText = Whitespace.normalizeWhitespace(editor, encodedText);
          const textHtml = Newlines.convert(normalizedText, Settings.getForcedRootBlock(editor), Settings.getForcedRootBlockAttrs(editor));const linkHtml = textHtml.replace(urlReg, '<a href="$1" target="_blank">$1</a>');
          pasteHtml(editor, linkHtml, internal);
        }
    } else {
